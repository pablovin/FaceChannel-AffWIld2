...
MODELTYPE = {'CPC_AV': 'CPC_AV'
                 }

def getModel(modelType):

        if modelType == MODELTYPE["CPC_AV"]:
...
def loadModel(directory):
   ...
   model = keras.models.load_model(directory, custom_objects=get_custom_objects_cpc())
...
def train(model, experimentFolder, trainGenerator, validationGenerator, trainDataLen, validationDataLen, batchSize = 64, epoches=10, params=[], verbose=1):
    ...
    allModelFolder = modelFolder + "/All/"
    encoderModelFolder = modelFolder + "/Encoder/"

    createFolders(allModelFolder)
    createFolders(encoderModelFolder)
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(filepath=allModelFolder, monitor='val_categorical_accuracy_custom',
                                        save_best_only=True, mode='min'),
        keras.callbacks.EarlyStopping(monitor='val_categorical_accuracy_custom', patience=4, mode='min'),
    ...
    model.fit(x=trainGenerator, batch_size=batchSize,
              ...
              callbacks=callbacks,
              steps_per_epoch=int(trainDataLen/batchSize),
              validation_steps=int(validationDataLen/batchSize)
              )

    print ("end training....")
    if len(os.listdir(allModelFolder)) > 0:
        model = loadModel(allModelFolder)

    # Saves the encoder
    encoder = model.layers[2].layer
    encoder.save(encoderModelFolder)
...
def network_cpc(crop_shape, n_crops, code_size, learning_rate, ks, n_neg, pred_dir):
    ...
    # encoder_model.get_layer(name="encoder_dense").trainable = True
...
def network_encoder(x, code_size):
    ...
    # resNet = tf.keras.applications.MobileNetV2(include_top=False, pooling='avg', input_shape=(32, 32, 3))(x)
    # resNet.trainable = False
    #
    # dense = keras.layers.Dense(code_size, activation="relu", name="encoder_dense")(resNet)
    #
    # return dense
    #
